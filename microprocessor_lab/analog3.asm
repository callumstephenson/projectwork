...
******************************
        ...
        cblock      0x20        ; the first data memory location where 
variables can be put
        ANALOG_VAL              ; the result of A/D conversion divided by 
32 is stored here
...
Program_Start
	...
	call    0x3ff           ; update factory calibrated oscillator: 
get the calibration value
        movwf   OSCCAL          ; update factory calibrated oscillator: 
store it in OSCCAL
        ...
        movwf   OPTION_REG      ; TMR0 prescaler: 1:32 (TMR0 will overflow 
in 8.2ms)
...
Main_Loop
        ...
        movf    ADRESH,W        ; Grab the high 8 bits of the analog 
result
        ...
        bsf     STATUS, RP0     ; Bank 1
        call    Get_Trisio      ; Use lookup table to get the value for 
the Tristate register
        movwf   TRISIO
        bcf     STATUS, RP0     ; Bank 0
        movf    ANALOG_VAL,w    ; grab analog value again
        call    Get_GPIO        ; Use another lookup table to get the 
value for the GPIO register
        movwf   GPIO
